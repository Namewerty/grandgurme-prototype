/* ============================================================================
   ...
   а клик её тут же закрывал — выглядело так, будто каталог не открывается
   вовсе. Теперь клик мышью по закрытому пункту открывает панель сразу
   (отложенное открытие по наведению отменяется), а по уже открытому —
   ничего не делает, двойной клик тоже. Закрывается панель уводом курсора,
   Esc, кликом мимо шапки, прокруткой и переходом по ссылке в панели.
   Клавиатура не меняется: Enter (клик с event.detail === 0) по-прежнему
   переключает панель. На устройствах без наведения шире 1024px (планшет
   горизонтально) касание тоже переключает: наведения там нет, и закрыть
   панель иначе нечем, кроме касания мимо.
...
export function initMegamenu({ header, items, onToggle }) {
  ...
  items.forEach((item) => {
    ...
    // Клик и Enter — одно событие; Enter отличаем по event.detail === 0.
    // На мобильном не мешаем ссылке работать.
    trigger.addEventListener('click', (event) => {
      if (!enabled()) return
      event.preventDefault()

      if (open !== item) {
        // Закрыто: открываем сразу, отложенное открытие по наведению гасим.
        clearTimers()
        openPanel(item)
        return
      }

      // Открыто. Клавиатура и касание переключают: там нет наведения, которым
      // панель закрылась бы сама. Мышь — ничего: панель остаётся открытой.
      const keyboard = event.detail === 0
      if (keyboard || !hoverable.matches) closeAll({ focusTrigger: true })
    })
