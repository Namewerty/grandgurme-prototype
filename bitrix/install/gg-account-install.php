...
//   опция главного модуля new_user_email_required = N
//
...
// ---- Почта у нового пользователя необязательна ----------------------------
//
// НАЙДЕНО 23.09.2026. По умолчанию Битрикс требует email у нового
// пользователя, и CUser::Add отвечал «Не указан email пользователя». Вход
// по номеру телефона почту не спрашивает — значит, кабинет не заводился
// ни для кого: на стенде не было ни одного покупателя с логином gg*, и вся
// ветка «новый номер» падала на шаге кода.
//
// Ставить вместо почты выдуманный адрес нельзя: на него ушло бы письмо
// о поступлении товара из листа ожидания. Почта остаётся необязательной,
// человек вписывает её в кабинете сам.

$emailOption = COption::GetOptionString('main', 'new_user_email_required', '');
if ($emailOption === 'N') {
    $say('Опция main/new_user_email_required — уже N');
} else {
    COption::SetOptionString('main', 'new_user_email_required', 'N');
    $say('Опция main/new_user_email_required — поставлена N (была «' . ($emailOption !== '' ? $emailOption : 'по умолчанию Y') . '»)');
}
