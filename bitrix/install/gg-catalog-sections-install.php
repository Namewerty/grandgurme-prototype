...
if ($iblockId) {
    ...
    /* ⚠ ПРОВЕРКА, КОТОРОЙ НЕ ХВАТИЛО 23.09.2026. Инфоблок создан второй
       версии: свойства хранятся в своей таблице b_iblock_element_prop_s<ID>,
       по колонке на свойство. Колонку создаёт CIBlockProperty::Add, и версию
       он спрашивает у CIBlock::GetArrayByID — то есть у КЕША. Если кеш
       инфоблоков устарел (запуск по SSH под deploy, который кеш почистить
       не может), нового инфоблока там нет, версия считается первой, колонки
       не создаются, значения уходят в общую таблицу — и форма записи
       в админке показывает пустые поля при живых текстах на сайте.

       Ловим это здесь, пока свойства ещё не заведены. */
    if ((int)CIBlockElement::GetIBVersion($iblockId) !== 2) {
        $out('');
        $out('СТОП: ядро видит инфоблок как первую версию — у него устарел кеш.');
        $out('Свойства сейчас завелись бы не в ту таблицу. Сбросьте кеш и запустите');
        $out('скрипт ещё раз: он идемпотентный, инфоблок второй раз не создастся.');
        $out('По SSH кеш сбрасывает сама команда npm run stand:php перед запуском.');
        return;
    }
}
