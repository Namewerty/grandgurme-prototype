...
function cmdPhp(file, { mode, allowDirty }) {
  ...
  /* КЕШ СБРАСЫВАЕТСЯ ДО ЗАПУСКА, А НЕ ТОЛЬКО ПОСЛЕ (23.09.2026).

     CLI работает под deploy, а папки кеша созданы php-fpm без групповой
     записи: свой кеш инфоблоков наш процесс ни почистить, ни перезаписать
     не может — он читает то, что оставил сайт. Один раз это уже стоило
     сломанного инфоблока: gg-catalog-sections-install.php создал инфоблок,
     тут же добавил в него свойства, а CIBlockProperty::Add спросил версию
     хранилища у CIBlock::GetArrayByID — та ответила по устаревшему списку,
     нового инфоблока в нём не было, и свойства завелись как у инфоблока
     первой версии: колонок в b_iblock_element_prop_s7 не появилось, значения
     легли в общую таблицу, а форма админки показывала пустые поля.

     Поэтому сначала просим сайт почистить кеш (метка + запрос), и только
     потом запускаем скрипт: он увидит базу такой, какая она есть. */
  ssh(
    `cd ${SITE} && mkdir -p local/gg-stand && : > local/gg-stand/cache-flush && ` +
      `curl -sk -o /dev/null ${CHECK_BASE}/`,
    { label: 'сброс кеша перед запуском' }
  )
